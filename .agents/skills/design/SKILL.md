...
- The fixed marketing `Navbar` (`components/common/navbar.tsx`) renders on public surfaces only, and asks `hidesNavbar` (`lib/shell.ts`) rather than `isAppShellPath` directly: two separate lists feed it. App-shell segments hide it because `SidebarShell` already carries the chrome; **chromeless** segments hide it because the page must offer no chrome at all. `/two-factor` is chromeless: it is reached mid-sign-in with no session, so a Login or Dashboard button there is either a way around the challenge or a link that cannot work. Every route whose layout renders `SidebarShell` already carries the brand, nav, and user menu in the sidebar, so the navbar hides itself there via `isAppShellPath` (`lib/shell.ts`), the single list of `(protected)` and `(console)` top-level segments: add a segment there whenever you add a top-level route to either group, or the navbar will sit fixed over the new page's sidebar header and `<h1>`. A public surface that does keep the navbar offsets around it (the docs sidebar's `md:pt-12`); app-shell pages add no top offset, so `PageShell` stays `p-4 sm:p-6` everywhere.
